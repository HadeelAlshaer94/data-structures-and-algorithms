...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let starWarssortedArr = starWarsArr.sort((a, b) => Number(a.height) > Number(b.height) ? -1 : 1);
  return starWarssortedArr;
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < components.length; i++) {
    let j = components[i].indexOf(' ') + 1;
    let updatedRec = components[i].slice(j);
    let index = updatedRec.indexOf(' ') + 1;
    result.push(updatedRec.slice(index));
  }
  return result;
};
