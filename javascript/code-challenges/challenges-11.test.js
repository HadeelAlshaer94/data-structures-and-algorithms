...
function transformToLis(obj) {
  ...
  Object.keys(obj).map(item => {
      newArr.push(`<li>${item}: ${obj[item]}</li>`);
  });
  return newArr;
}
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  return data.filter(itemY =>
      (itemY.gender === 'male') || (itemY.gender === 'female')).map(itemX =>
          itemX.name).join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let star = data.reduce((item, i) => {
      if (parseInt(item.height) < parseInt(i.height)) {
          return item;
      }
      return i;
  })
