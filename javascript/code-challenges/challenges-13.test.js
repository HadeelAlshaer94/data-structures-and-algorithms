...
const standardizePhoneNumbers = (arr) => {
   // Solution code here...
  let num = arr.map(item=> item.replace(/[^0-9\.]+/g,''))
  return num;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let strinOdd=str.split('');
  let results=strinOdd.filter((item,index)=>{
    if(index %2 !=0)
    {
      return item;
    }
