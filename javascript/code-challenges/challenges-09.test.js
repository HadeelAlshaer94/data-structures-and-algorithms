...
const checkValues = (obj, value) => {
  // Solution code here...
  if (Object.values(obj).includes(value)) { return true; } else { return false; }
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  let myArray = [];
  Object.keys(obj).forEach((prop) => { myArray.push(`${prop}: ${obj[prop]}`); });
  return myArray;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.map((person) => {
    if (person.name === character) {
      if (Object.values(person).includes(person.children)) {
        myArr = true;
      } else { myArr = false; }
    }
  });
