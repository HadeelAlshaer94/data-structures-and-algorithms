...
const addQuestion = (arr) => {
  let addQuestionArr = arr.map(value => {
    return (value + "!");
  });
  return addQuestionArr;
};
...
const forLoopTwoToThe = (arr) => {
  let forLoopTwoToTheArr = [];
  for (let i = 0; i < arr.length; i++) {
    let newEl = Math.pow(2, arr[i]);
    forLoopTwoToTheArr.push(newEl);
  }
  return forLoopTwoToTheArr;
};
...
const forEachTwoToThe = (arr) => {
  let forEachTwoToTheArr = [];
  arr.forEach(element => {
    let newEl = Math.pow(2, element);
    forEachTwoToTheArr.push(newEl);
  });
  return forEachTwoToTheArr;
};
